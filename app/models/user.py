...
class User(Base):
    ...
    first_name: Mapped[str] = mapped_column(String, nullable=True) # Додали ім'я
    last_name: Mapped[str] = mapped_column(String, nullable=True)  # Додали прізвище
    phone: Mapped[str] = mapped_column(String, nullable=True)      # Додали телефон
...
class Appointment(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True, index=True)
    master_id: Mapped[int] = mapped_column(Integer, default=1)
    business_id: Mapped[int] = mapped_column(ForeignKey("businesses.id"))
