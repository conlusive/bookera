...
export default function HomePage() {
  ...
  // Ref-и для ГОЛОВНОГО БАНЕРА (HERO)
  ...
  }, [searchWhat, searchWhere, searchDate, activeCategory, isAuthModalOpen, businesses]);
  ...
  const filteredBusinesses = businesses
    .filter(biz => {
      ...
      if (activeCategory !== 'all' && !appliedSearch) {
        ...
        const searchableText = `${biz.category || ''} ${biz.name} ${biz.description || ''}`.toLowerCase();
      ...
      if (appliedSearch) {
        ...
        const fullText = `${biz.category || ''} ${biz.name} ${biz.description || ''} ${biz.address || ''}`.toLowerCase();
    ...
    .sort((a, b) => {
      if (sortBy === 'rating') {
        const ratingA = parseFloat(a.reviews_rank) || parseFloat(a.rating) || 0;
        const ratingB = parseFloat(b.reviews_rank) || parseFloat(b.rating) || 0;
  ...
  return (
    <div style={{ backgroundColor: '#ffffff', minHeight: '100vh', fontFamily: 'system-ui, -apple-system, sans-serif', color: '#222222', overflowX: 'hidden' }}>
      ...
      <style>{`
        ...
        .tour-card-img-wrapper { width: 100%; height: 170px; position: relative; overflow: hidden; }
      ...
      <header className={`main-header ${scrollState}`}>
        <div className="container" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', height: '100%', gap: '2rem' }}>
          ...
          {/* 2. ПОШУКОВИЙ БАР В ХЕДЕРІ */}
          <div style={{
            flex: '1 1 auto', display: 'flex', justifyContent: 'center', /* 🟢 Змінено на center, щоб посунути правіше від логотипу */
            opacity: isHeaderDark ? 1 : 0,
            visibility: isHeaderDark ? 'visible' : 'hidden',
            transform: isHeaderDark ? 'translateY(0)' : 'translateY(-10px)',
            transition: 'all 0.3s cubic-bezier(0.25, 0.8, 0.25, 1)'
          }}>
            <div style={{
              display: 'flex', alignItems: 'center', backgroundColor: '#ffffff', padding: '2px', borderRadius: '12px',
              width: '100%', maxWidth: '650px',
              boxSizing: 'border-box',
              border: '1px solid #e2e8f0', boxShadow: '0 8px 24px rgba(0,0,0,0.06)',
              position: 'relative'
            }}>

              <div ref={headerWhatRef} style={{ flex: 1.3, display: 'flex', alignItems: 'center', padding: '0 0.5rem 0 1rem', position: 'relative' }}>
                ...
                <input
                  ...
                  value={searchWhat}
                  onChange={(e) => { setSearchWhat(e.target.value); setIsWhatOpen(true); }}
                  onFocus={() => setIsWhatOpen(true)}
                  onKeyDown={(e) => e.key === 'Enter' && handleSearch()}
                  style={{ width: '100%', border: 'none', outline: 'none', fontSize: '0.95rem', color: '#111827', backgroundColor: 'transparent' }}
                />
                {isWhatOpen && whatSuggestions.length > 0 && (
              ...
              <div ref={headerWhereRef} style={{ flex: 1, display: 'flex', alignItems: 'center', padding: '0 0.5rem', position: 'relative' }}>
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#94a3b8" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ marginRight: '0.6rem', flexShrink: 0 }}>
                ...
                <input
                  ...
                  value={searchWhere}
                  onChange={(e) => { setSearchWhere(e.target.value); setIsWhereOpen(true); }}
                  onFocus={() => setIsWhereOpen(true)}
                  onKeyDown={(e) => e.key === 'Enter' && handleSearch()}
                  style={{ width: '100%', border: 'none', outline: 'none', fontSize: '0.95rem', fontWeight: '600', color: '#111827', backgroundColor: 'transparent' }}
                />
                {isWhereOpen && whereSuggestions.length > 0 && (
                  <div className="search-dropdown anim">
              ...
              <div ref={headerDateRef} onClick={() => setIsDateOpen(!isDateOpen)} style={{ flex: 0.8, display: 'flex', alignItems: 'center', padding: '0 1.25rem 0 0.5rem', cursor: 'pointer', position: 'relative' }}>
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#94a3b8" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ marginRight: '0.6rem', flexShrink: 0 }}>
                ...
                <span style={{ color: searchDate ? '#111827' : '#64748b', fontSize: '0.95rem', fontWeight: searchDate ? '600' : '400', flexGrow: 1, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                  {searchDate ? new Date(searchDate).toLocaleDateString('uk-UA', { day: 'numeric', month: 'short' }) : 'Будь-коли'}
                </span>

                {isDateOpen && (
                  <div className="search-dropdown anim" style={{ maxHeight: 'none', overflowY: 'visible', padding: '1.5rem', width: '340px', right: 0, left: 'auto', top: 'calc(100% + 14px)', borderRadius: '20px', border: '1px solid #e2e8f0', boxShadow: '0 20px 40px rgba(0,0,0,0.1)' }} onClick={(e) => e.stopPropagation()}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                      <button onClick={() => setCurrentMonth(new Date(currentMonth.getFullYear(), currentMonth.getMonth() - 1, 1))} style={{ width: '36px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#fff', border: '1px solid #e2e8f0', borderRadius: '10px', cursor: 'pointer', color: '#64748b', transition: '0.2s' }} onMouseOver={e=>e.currentTarget.style.background='#f8fafc'} onMouseOut={e=>e.currentTarget.style.background='#fff'}>
                        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6"></polyline></svg>
                      </button>
                      <div style={{ fontWeight: '800', color: '#0f172a', fontSize: '1.05rem', textTransform: 'capitalize' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '4px', textAlign: 'center', fontSize: '0.8rem', color: '#94a3b8', fontWeight: '700', marginBottom: '0.8rem', textTransform: 'uppercase' }}>
              ...
              {/* 🟢 ПОВЕРНУТА КНОПКА ПОШУКУ (ЛУПА) */}
              <button onClick={handleSearch} style={{ width: '42px', height: '42px', borderRadius: '10px', backgroundColor: '#111827', border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0, transition: '0.2s', marginLeft: '6px' }} onMouseOver={e=>e.currentTarget.style.backgroundColor='#334155'} onMouseOut={e=>e.currentTarget.style.backgroundColor='#111827'}>
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#ffffff" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="8"></circle><line x1="21" y1="21" x2="16.65" y2="16.65"></line></svg>
              </button>

            </div>
          </div>

          {/* 3. НАВІГАЦІЯ ТА ПРОФІЛЬ */}
            ...
            {isLoggedIn ? (
              <div style={{ position: 'relative' }} ref={profileRef}>
                <div onClick={() => setIsProfileOpen(!isProfileOpen)} style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.6rem', userSelect: 'none', padding: '0.3rem', borderRadius: '20px', transition: '0.2s' }}>
                  <span style={{ color: isHeaderDark ? '#111827' : '#ffffff', transition: '0.2s', fontSize: '0.95rem', fontWeight: '600', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: '120px' }}>{userName}</span>
                  <div style={{
      ...
      <section style={{ position: 'relative', width: '100%', height: '540px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        ...
        <div className="reveal-on-scroll" style={{ position: 'relative', zIndex: 50, maxWidth: '1340px', width: '100%', margin: '0 auto', padding: '4rem 4rem 0 4rem', boxSizing: 'border-box', textAlign: 'center' }}>
          ...
          <div style={{ backgroundColor: '#ffffff', padding: '6px', borderRadius: '16px', maxWidth: '750px', width: '100%', margin: '0 auto', display: 'flex', alignItems: 'center', boxSizing: 'border-box', boxShadow: '0 20px 40px rgba(0,0,0,0.15)', position: 'relative', zIndex: 100 }}>

            <div ref={heroWhatRef} style={{ flex: 1.3, position: 'relative', display: 'flex', alignItems: 'center', padding: '0 0.5rem 0 1rem' }}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#94a3b8" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ marginRight: '0.6rem', flexShrink: 0 }}>
                <circle cx="11" cy="11" r="8"></circle>
                <line x1="21" y1="21" x2="16.65" y2="16.65"></line>
              </svg>
              <input
                ...
                onFocus={() => setIsWhatOpen(true)}
                onKeyDown={(e) => e.key === 'Enter' && handleSearch()}
                style={{ width: '100%', border: 'none', outline: 'none', color: '#222222', fontSize: '0.95rem', backgroundColor: 'transparent', padding: '0.4rem 0' }}
              />
              {isWhatOpen && whatSuggestions.length > 0 && (
                <div className="search-dropdown anim">
                  {whatSuggestions.map((item, idx) => (
            ...
            <div ref={heroWhereRef} style={{ flex: 0.9, position: 'relative', display: 'flex', alignItems: 'center', padding: '0 0.75rem' }}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#94a3b8" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ marginRight: '0.6rem', flexShrink: 0 }}>
                <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"></path>
                <circle cx="12" cy="10" r="3"></circle>
              </svg>
              <input
                ...
                onFocus={() => setIsWhereOpen(true)}
                onKeyDown={(e) => e.key === 'Enter' && handleSearch()}
                style={{ width: '100%', border: 'none', outline: 'none', color: '#222222', fontSize: '0.95rem', fontWeight: '600', backgroundColor: 'transparent', padding: '0.4rem 0' }}
              />
              {isWhereOpen && whereSuggestions.length > 0 && (
                <div className="search-dropdown anim">
                  {whereSuggestions.map((city) => (
            ...
            <div ref={heroDateRef} style={{ flex: 0.8, position: 'relative', display: 'flex', alignItems: 'center', padding: '0 0.5rem', cursor: 'pointer' }} onClick={() => setIsDateOpen(!isDateOpen)}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#94a3b8" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ marginRight: '0.6rem', flexShrink: 0 }}>
                <rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect>
                <line x1="16" y1="2" x2="16" y2="6"></line>
                <line x1="8" y1="2" x2="8" y2="6"></line>
              ...
              <span style={{ color: searchDate ? '#222222' : '#64748b', fontSize: '0.95rem', fontWeight: searchDate ? '600' : '400', flexGrow: 1, whiteSpace: 'nowrap' }}>
                {searchDate ? new Date(searchDate).toLocaleDateString('uk-UA', { day: 'numeric', month: 'long' }) : 'Будь-коли'}
              </span>

              {isDateOpen && (
                <div className="search-dropdown anim" style={{ maxHeight: 'none', overflowY: 'visible', padding: '1.5rem', width: '340px', right: 0, left: 'auto', top: '130%', borderRadius: '20px', border: '1px solid #e2e8f0', boxShadow: '0 20px 40px rgba(0,0,0,0.1)' }} onClick={(e) => e.stopPropagation()}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                    <button onClick={() => setCurrentMonth(new Date(currentMonth.getFullYear(), currentMonth.getMonth() - 1, 1))} style={{ width: '36px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#fff', border: '1px solid #e2e8f0', borderRadius: '10px', cursor: 'pointer', color: '#64748b', transition: '0.2s' }} onMouseOver={e=>e.currentTarget.style.background='#f8fafc'} onMouseOut={e=>e.currentTarget.style.background='#fff'}>
                      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6"></polyline></svg>
                    </button>
      ...
      <section className="reveal-on-scroll" style={{ paddingBottom: '5rem' }} id="salons-section">
        <div className="container">
          ...
          {loading ? (
            <div style={{ color: '#64748b', padding: '4rem 0', textAlign: 'center', fontWeight: '650', position: 'relative', zIndex: 10 }}>Завантаження актуальної бази салонів...</div>
                  ...
                  <Link key={biz.id} href={`/salon/${biz.id}`} className="tour-card">
                    ...
                    <div className="tour-card-content">
                      ...
                      <div className="tour-tags">
                        <span className="tour-tag-pill">Стрижка</span>
                        <span className="tour-tag-pill">Догляд</span>
                        <span className="tour-tag-pill">+4</span>
                      </div>
        ...
        <div className="container">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6rem', alignItems: 'center' }}>
            ...
            <div className="reveal-on-scroll delay-100" style={{ position: 'relative', height: '450px', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
              ...
              {/* Плаваючі віджети поверх дашборду */}
